...
### Admin3-2. 訓練作成（ステップ1：メール生成）

- `/admin/drills/create`
- 役割：
  - 新規訓練の作成・メール生成・保存（ステップ1）
  - ステータス遷移：New → Draft
- 内容：
  - **シナリオ選択**
    - MVP では固定シナリオをドロップダウンで選択
  - **AI生成（メール文面）**（LangChain + ローカルLLM）
    - **事前プロンプト機能**：生成時に「AIへの追加指示（任意）」を入力可能。入力内容はプロンプトの「追加指示」としてLLMに渡され、差出人・トーン等に反映される（詳細は `doc/features/promptStrategy.md`）。
    - 訓練メール：件名/本文/誘導テキスト等を自動生成
  - **メール内容の表示・編集**
    - AI生成結果を表示し、**手動で編集可能**
    - **「AI再生成」および「AIで再編集」機能**
  - **「同意して保存」ボタン** 
    - 必須項目を揃えてDraft ステータスで保存し、自動的に **`/admin/drills/[id]/edit` （ステップ2）へリダイレクト**する。
    - ※この画面にはクイズ生成のUIは存在しない。
...
### Admin3-4. 訓練編集・クイズ生成・配信（ステップ2）

- `/admin/drills/[id]/edit`
- 役割：
  - 作成した訓練メールに紐づく**クイズの生成・管理**、および最終的な配信準備
  - ステータス遷移：Draft → Deliverable → Delivering
- 内容：
  - 現在のステータスを表示（Draft / Deliverable / Delivering）
  - **メール内容の再編集・生成**
    - 保存済みのメール内容を表示し、手動での再編集やAI推敲が可能
    - 新規作成時と同様に、ユーザープロンプト不要でAIによるメール文面のイチからの生成も可能
  - **クイズ内容の表示・編集・生成**
    - **「クイズをAI一括生成」ボタン**：生成された本文をもとに、クイズを3問生成する。
    - **「AIで1問追加」ボタン**：既存の問題と被らないように、AIで新しく1問だけ追加生成する。
    - 既存の問題文/選択肢/正解/解説を表示し、**手動で編集・追加可能**
  - **「保存」ボタン** → バリデーション無し、Draftステータスで保存
  - **「配信可能にする」ボタン** → メールとクイズの必須項目をすべてチェック
    - ✅ 成功：条件を満たせば **Deliverable へ昇格**
    - ❌ 失敗：エラーメッセージ表示
  - **「配信する」ボタン**（Deliverable 状態のみ有効）
    - 対象者選択（全員 / 特定の個人 / ランダム選出） / チャネル選択（メール・Slack） / 送信タイミング
    - 送信実行 → **Delivering へ遷移** → `/admin/drills/[id]` へリダイレクト
  - **「配信停止」ボタン**（Delivering 状態のみ有効）
    - 配信を停止 → **Stopped へ遷移** → `/admin/drills/[id]` へリダイレクト

**クイズ必須項目**（Deliverable昇格時）:
  - 問題文（Questions）：空でない
  - 出題形式（Question Type）：`single_choice` / `multiple_choice`
  - 選択肢：2個以上、各選択肢が空でない。正解は1つ以上
  - 解説（Explanation）：空でない
...
- 不合格 → **不合格・再受験用の専用画面**を表示してから再受験（受験回数制限なし）
  - 表示内容：不合格表示、スコア・受験回数、改善ポイント（LLM 分析）、「再受験する」ボタン
  - デザイン：`doc/design/user.pen` の「Quiz Failed Retry」画面（同一URLで状態切り替えでも可）
  - LLM がこれまでの受験パターンを分析し「〇〇が弱いようです」という改善ポイントをフィードバック
- 合格 → 合格画面（General2-3）へ
  - 受験履歴全体の分析フィードバック表示
...
- 合格点未満の場合は不合格・再受験画面を表示し、そこから再挑戦可能
