...
* フォームの「入力形式」の検証が中心。安全性のためのビジネスルール判定は domain に置く（後述「validators と domain の違い」参照）
...
* **安全性のための検証**（例：プロンプトインジェクション検出、許可されない表現の判定）もここに置く。zod を使わない純粋関数で表現する

#### validators と domain の「バリデーション」の違い（初学者向け）

「バリデーション」という言葉は **validators/** と **domain/** の両方で出てきますが、役割が違います。どちらに書くか迷ったときの目安です。

| 観点 | validators/ | domain/ |
|------|-------------|---------|
| **何を置くか** | zod のスキーマ（型＋検証ルール） | 純粋関数（値オブジェクトやビジネスルール） |
| **主な目的** | フォームの入力形式チェック（UX） | 安全性・ビジネスルールの保証 |
| **利用場所** | UI（即時フィードバック）と Server Action（再検証）で共有 | usecase や Server Action から呼び出す |
| **例** | 必須入力・文字数・メール形式を zod で定義 | 「この文字列は許可するか」（インジェクション疑いの検出など）を関数で判定 |

**具体例**：ユーザーが入力した「追加指示」を検証する場合

* **validators/** … 「500文字以内」「必須かどうか」などを zod で定義し、UI と Server Action の両方で同じスキーマを使う。
* **domain/** … 「この文言はプロンプトインジェクションとみなして拒否する」といった**ビジネスルール**を、正規表現や純粋関数で実装する。zod も React も使わない。

両方使う場合は、Server Action で validators の zod を通したあと、domain の検証関数を呼んで最終的な許可・拒否を決める、という流れになります。
...
* **domain/usecase でも最終検証は行う**
  ...
  * 「validators と domain のどちらに書くか」の目安は、上記 feature 配下の「validators と domain の『バリデーション』の違い（初学者向け）」を参照
