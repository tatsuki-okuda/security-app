  ...
  if (!result.ok) {
    ...
    return (
      <main className="mx-auto mt-12 max-w-4xl space-y-3 px-4 sm:px-0">
        <h1 className="text-2xl font-semibold text-text-primary">参加者詳細</h1>
  ...
  return (
    <main className="mx-auto mt-12 max-w-4xl space-y-6 px-4 sm:px-0">
      <header className="space-y-2">
        <Link href="/admin/users" className="text-sm font-semibold text-text-secondary">
          ← 一覧へ戻る
        </Link>
        <h1 className="text-2xl font-semibold text-text-primary">参加者詳細</h1>
      </header>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-text-primary">訓練履歴</h2>
        <div className="overflow-hidden rounded-2xl border border-border bg-surface/80">
          <table className="w-full text-left text-sm">
            <thead className="bg-bg text-xs uppercase tracking-wide text-text-secondary">
              <tr>
            ...
            <tbody>
              {user.drills.map((drill) => (
                <tr key={drill.drillId} className="border-t border-border">
                  <td className="px-4 py-3">
                    <Link href={`/admin/drills/${drill.drillId}`} className="font-semibold text-text-primary">
                      {drill.title}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3 text-text-secondary">{drill.clickedAt ? 'クリック済み' : '未クリック'}</td>
                  <td className="px-4 py-3 text-text-secondary">
